...
const EventSearchResults: React.FC = () => {
    ...
    return (
        <>
            <CssBaseline/>
            <TopContent
                mainText={events.length > 0 ? "We Discovered Outstanding Results for You..." : `We are Looking for Results for You...`}
                subText=''/>
            <Box display="flex" justifyContent="center">
                {isLoading ? (
                    ...
                    <UpcomingEvents events={events}
                                    title={`${eventData?.performer} Events${eventData?.toCity ? ` in ${eventData?.toCity}` : ''}`}/>
                ) : (
                    <Stack justifyContent={'center'} alignItems={'center'} sx={{p: 8}}>
                        <Typography variant="h4" color="textSecondary" textAlign="center">Looks like there are no events
